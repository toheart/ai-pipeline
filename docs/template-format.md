...
> **注意**：Cursor 的 Task 工具不会自动加载 `.cursor/agents/*.md` 中的自定义指令（[详情](../docs/subagent-instruction-loading-zh.md)）。`generate-orchestrator.ts` 在生成 SKILL.md 时会自动为每个 Agent 阶段注入读取指令，确保 SubAgent 启动后加载项目级定义。
