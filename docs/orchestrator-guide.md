...
- 委托给哪个 SubAgent
- **prompt 开头必须包含指令加载**：`操作前必须先读取 .cursor/agents/<agent-name>.md 并严格遵循其中的全部指令。`（Task 工具不会自动加载 `.cursor/agents/*.md` 中的自定义指令，必须显式注入）
...
- 用 ASCII 图展示并行结构
- 分别说明每个并行 Agent 的 prompt 内容，**每个 Agent 的 prompt 开头必须包含指令加载**：`操作前必须先读取 .cursor/agents/<agent-name>.md 并严格遵循其中的全部指令。`
...
- **重要**：Task 工具的 `subagent_type` 不会自动加载 `.cursor/agents/*.md` 中的自定义指令（平台限制）。orchestrator 必须在每个 SubAgent 的 Task prompt 开头显式注入读取指令。`generate-orchestrator.ts` 已自动处理此逻辑。详见 [SubAgent 指令加载的坑](../docs/subagent-instruction-loading-zh.md)
