#!/usr/bin/env tsx
/**
 * Orchestrator Skill 生成脚本
 *
 * 从 .pipeline/*.ts 定义编译出 manifest.json + orchestrator SKILL.md。
 * 整合了原 bin/commands/generate.ts 和 src/generator/skill-generator.ts 的功能。
 *
 * 用法：
 *   tsx scripts/generate-orchestrator.ts [--adapter <name>] [file...]
 *
 * 参数：
 *   --adapter    IDE 适配器名称（默认 cursor）
 *   file...      Pipeline 定义文件，默认扫描 .pipeline/*.ts
 */
...
export async function generateOrchestrator(args: string[]): Promise<void> {
...
// CLI 入口
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1]?.endsWith("generate-orchestrator.ts")) {
  generateOrchestrator(process.argv.slice(2));
}
